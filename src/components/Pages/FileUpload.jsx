...
const FileUpload = () => {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        const response = await fetch('https://dunlin-backend-qa9i.onrender.com/upload/history', {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://dunlin-backend-qa9i.onrender.com/upload', {
        method: 'POST',
        body: formData,
        headers: {
          'Authorization': `Bearer ${token}`
  ...
  const fetchSummaryById = async (_id) => {
    try {
      const response = await fetch(`https://dunlin-backend-qa9i.onrender.com/upload/history/${_id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const deleteSummaryById = async (id) => {
    try {
      const response = await fetch(`https://dunlin-backend-qa9i.onrender.com/upload/history/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
